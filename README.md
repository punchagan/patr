...
If your published site sits behind a login (subscribers only), set `subscribers_only = true` under `[params.patr]` in `hugo.toml`. Mail clients fetch emailed images without a login, so images linked from a gated site would break; in this mode Patr embeds them in the email as base64 (like email-only mode), while still publishing the edition and keeping the "View in browser" link. Patr doesn't set up the login itself, and this isn't in the Settings panel yet — edit `hugo.toml` directly. Because the site is behind a login, Patr can't check that an edition is live (its check fetches the page without one and would only see the login wall), so in this mode that check is skipped: Patr shows a note that you should confirm the deploy yourself, and **Send All** stays enabled (unsaved and unpublished-local-changes checks still apply). It applies to the whole site: marking individual editions private isn't supported yet.
