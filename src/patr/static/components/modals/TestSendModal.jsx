...
export default function TestSendModal({ slug, onClose, onSent }) {
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <div style={{ maxHeight: 260, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16, fontSize: 13 }}>
        {error ? (
          ...
          <>
            ...
            {contacts.map(c => (
              <label key={c.email} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                <input type="checkbox" checked={!!checked[c.email]} onChange={() => toggle(c.email)} />
                <span>
                  {c.name || c.email}
                  {c.name && <span style={{ color: 'var(--text-secondary)', fontSize: 11 }}> &lt;{c.email}&gt;</span>}
                </span>
              </label>
            ))}
          </>
