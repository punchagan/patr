...
export default function ConfirmModal({ slug, title, onClose, onConfirm }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/check-deployment/${slug}`).then(r => r.json()).then(setDeployment)
  }, [slug])
  ...
  if (deployment && !emailOnly) {
    ...
    if (deployment.unpushed)
      warnings.push('The edition has local changes that haven\'t been published to your site yet — hit Publish first.')
  }
    ...
    <Modal onClose={onClose}>
      ...
      {hasMissingImages && (
        <div className="warning-box">
          <strong>Missing images</strong> — these files are referenced but don't exist in the edition folder:
