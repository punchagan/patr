...
def test_creates_section_when_absent(hugo_toml):
...
def test_adds_key_to_existing_section(hugo_toml):
...
def test_updates_existing_key(hugo_toml):
    hugo_toml.write_text('[params.patr]\n  name = "Old Name"\n')
...
def test_preserves_other_keys_in_section(hugo_toml):
    hugo_toml.write_text('[params.patr]\n  name = "Old"\n  other = "keep"\n')
    save_hugo_patr_params({"name": "New"})
    text = hugo_toml.read_text()
...
def test_saves_multiple_keys_at_once(hugo_toml):
    hugo_toml.write_text("[params.patr]\n")
