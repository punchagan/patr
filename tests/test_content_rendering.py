...
def test_render_md_image_without_alt_stays_plain() -> None:
    html = render_md("![](photo.jpg)")
    assert "<figure>" not in html
    assert "<img" in html


def test_render_md_image_with_alt_becomes_figure() -> None:
    html = render_md("![A cat](photo.jpg)")
    assert "<figure>" in html
    assert "<figcaption>A cat</figcaption>" in html


def test_render_md_attr_list_syntax_not_processed() -> None:
...
def test_absolutify_root_relative() -> None:
...
def test_absolutify_relative() -> None:
...
def test_absolutify_leaves_absolute_alone() -> None:
...
def test_email_html_contains_greeting() -> None:
    post = make_post()
...
def test_email_html_image_with_alt_becomes_figure() -> None:
...
def test_email_html_empty_body() -> None:
    post = make_post(body="")
    html = build_email_html("test-ed", post, FOOTER_MD, HUGO_CONFIG)
    assert "Hi," in html  # renders without crashing


def test_email_html_no_intro() -> None:
    post = make_post(intro="")
    html = build_email_html("test-ed", post, FOOTER_MD, HUGO_CONFIG)
    assert "font-style:italic" not in html


def test_email_html_base_url_trailing_slash_not_doubled() -> None:
...
def test_email_html_multiple_images_all_absolutified() -> None:
...
def test_email_html_empty_base_url_image_paths_are_not_absolute() -> None:
    post = make_post(body="![Cat](cat.jpg)")
...
def test_email_html_whitespace_only_name_falls_back_to_generic_greeting() -> None:
    post = make_post()
    html = build_email_html(
...
def test_preview_html_renders_body() -> None:
    post = make_post(body="Read all about **it**.")
    html = build_email_html("test-ed", post, FOOTER_MD, LOCALHOST_CONFIG)
    assert "<strong>it</strong>" in html


def test_preview_html_renders_intro() -> None:
    post = make_post(intro="A quick note.")
    html = build_email_html("test-ed", post, FOOTER_MD, LOCALHOST_CONFIG)
    assert "A quick note." in html


def test_preview_html_renders_footer() -> None:
    post = make_post()
    html = build_email_html("test-ed", post, FOOTER_MD, LOCALHOST_CONFIG)
    assert "Unsubscribe" in html


def test_preview_html_no_footer_when_empty() -> None:
    post = make_post()
    html = build_email_html("test-ed", post, "", LOCALHOST_CONFIG)
    assert "Unsubscribe" not in html


def test_preview_html_image_with_alt_becomes_figure() -> None:
    post = make_post(body="![A sunset](sunset.jpg)")
    html = build_email_html("test-ed", post, FOOTER_MD, LOCALHOST_CONFIG)
    assert "<figure" in html
    assert "A sunset" in html
...
def test_preview_html_relative_image_uses_localhost() -> None:
    post = make_post(body="![img](photo.jpg)")
    html = build_email_html("test-ed", post, FOOTER_MD, LOCALHOST_CONFIG)
    assert "http://127.0.0.1:5000/newsletter/test-ed/photo.jpg" in html


def test_preview_html_root_relative_image_uses_localhost() -> None:
    post = make_post(body="![logo](/images/logo.png)")
    html = build_email_html("test-ed", post, FOOTER_MD, LOCALHOST_CONFIG)
...
def test_email_only_omits_view_in_browser(tmp_path) -> None:
    post = make_post()
    html = build_email_html(
...
def test_email_only_embeds_relative_image_as_data_uri(tmp_path) -> None:
...
def test_email_only_does_not_embed_missing_image(tmp_path) -> None:
...
def test_email_only_leaves_external_images_alone(tmp_path) -> None:
...
def test_email_only_embeds_root_relative_image_from_static(tmp_path) -> None:
...
def test_email_only_embedded_image_alt_is_filename(tmp_path) -> None:
...
def test_email_only_leaves_missing_root_relative_alone(tmp_path) -> None:
...
def test_normal_mode_still_has_view_in_browser() -> None:
    post = make_post()
    html = build_email_html("test-ed", post, FOOTER_MD, HUGO_CONFIG)
...
def test_img_max_width_constrained() -> None:
    post = make_post(body="![img](photo.jpg)")
